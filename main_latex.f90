...
! Model Parameters
    ...
    real(dp), parameter :: delta = 0.12_dp
...
! General Equilibrium Prices and Aggregates
    real(dp) :: r, w, ky0, ky1, kl, cap, lab, kd
! Variables
...
kd = ((r+delta)/alpha)**(1/(alpha-1))*lab

if (myrank == root) then
    write (*,'(A10, F7.4, A12, F7.4, A8, F7.4)') 'capital =', cap, &
            'interest =', 1+r, 'wage =', w
    write (*,'(A7,F12.6,A7,F12.6,A10,F12.6)') 'kd=', kd, 'ks=', cap, 'kyratio =', ky0
endif
